...
class UtilsCapsNet:
    @staticmethod
    def update_locally_constrained_routing(votes, biases, logit_shape, num_dims, input_dim, output_dim,
                       num_routing):
        if num_dims == 6:
        ...
        def _body(i, logits, activations):
            ...
            activation = UtilsCapsNet.squash(preactivate)
    ...
    def squash(input_tensor, axis=-1):
        norm = tf.norm(input_tensor, axis, keepdims=True)
...
class CapsuleLayer(layers.Layer):
    ...
    def call(self, inputs, training=None):
        ...
        for i in range(self.routings):
            ...
            outputs = UtilsCapsNet.squash(K.batch_dot(c, inputs_hat, [2, 2]))  # [None, 10, 16]
...
def PrimaryCaps(inputs, dim_capsule, n_channels, kernel_size, strides, padding):
    ...
    return layers.Lambda(UtilsCapsNet.squash, name='primarycaps_squash')(outputs)
...
class ConvCapsuleLayer(layers.Layer):
    ...
    def call(self, input_tensor, training=None):
        ...
        activations = UtilsCapsNet.update_locally_constrained_routing(
            votes=votes,
