...
from libs.darknet53.layers import darknet_conv
...
def yolo_v4(size=None, channels=3, anchors=yolo_anchors,
    ...
    # 19x19 head
    y19 = darknet_conv(x, 512, 1)
    y19 = darknet_conv(y19, 1024, 3)
    y19 = darknet_conv(y19, 512, 1)
    ...
    y19_upsample = darknet_conv(y19, 256, 1)
    y19_upsample = UpSampling2D(2)(y19_upsample)

    # 38x38 head
    y38 = darknet_conv(x38, 256, 1)
    y38 = Concatenate()([y38, y19_upsample])
    y38 = darknet_conv(y38, 256, 1)
    y38 = darknet_conv(y38, 512, 3)
    y38 = darknet_conv(y38, 256, 1)
    y38 = darknet_conv(y38, 512, 3)
    y38 = darknet_conv(y38, 256, 1)

    y38_upsample = darknet_conv(y38, 128, 1)
    y38_upsample = UpSampling2D(2)(y38_upsample)

    # 76x76 head
    y76 = darknet_conv(x76, 128, 1)
    y76 = Concatenate()([y76, y38_upsample])
    y76 = darknet_conv(y76, 128, 1)
    y76 = darknet_conv(y76, 256, 3)
    y76 = darknet_conv(y76, 128, 1)
    y76 = darknet_conv(y76, 256, 3)
    y76 = darknet_conv(y76, 128, 1)

    # 76x76 output
    y76_output = darknet_conv(y76, 256, 3)
    output_2 = darknet_conv(y76_output, len(yolo_anchors) * (classes + 5), 1, batch_norm=False)

    # 38x38 output
    y76_downsample = darknet_conv(y76, 256, 3, strides=(2, 2))
    ...
    y38_output = darknet_conv(y38, 512, 3)
    output_1 = darknet_conv(y38_output, len(yolo_anchors) * (classes + 5), 1, batch_norm=False)

    # 19x19 output
    y38_downsample = darknet_conv(y38, 512, 3, strides=(2, 2))
    ...
    y19_output = darknet_conv(y19, 1024, 3)
    output_0 = darknet_conv(y19_output, len(yolo_anchors) * (classes + 5), 1, batch_norm=False)
