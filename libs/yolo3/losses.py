...
def yolo_loss(anchors, classes=80, ignore_thresh=0.5, label_smoothing=0, elim_grid_sense=False, use_focal_loss=False,
    ...
    def calc_yolo_loss(y_true, y_pred):
        ...
        if use_giou_loss:
            ...
            wh_loss = obj_mask * box_loss_scale * 0.5 * tf.square(true_wh - pred_wh)
            xy_loss = keras.sum(xy_loss) / batch_size
            wh_loss = keras.sum(wh_loss) / batch_size
            location_loss = xy_loss + wh_loss

        # 6. sum over (batch, gridx, gridy, anchors) => (batch, 1)
        # obj_loss = tf.reduce_sum(obj_loss, axis=(1, 2, 3))
        # class_loss = tf.reduce_sum(class_loss, axis=(1, 2, 3))
        obj_loss = keras.sum(obj_loss) / batch_size
        class_loss = keras.sum(class_loss) / batch_size
