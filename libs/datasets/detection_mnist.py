...
def _prepare_annotation(filename, path, labels, bboxes):
    with open(filename, 'a') as file:
        string = str(path)
        bboxs = ''
        for i in range(len(bboxes)):
            bbox_string = f'{int(bboxes[i][0])},{int(bboxes[i][1])},' \
                          f'{int(bboxes[i][2])},{int(bboxes[i][3])},{labels[i]}'
            bboxs += ' ' + bbox_string
        string += bboxs
        file.write(string + '\n')
    ...
    for image_id in tqdm.trange(num_images, desc=f"Generating dataset, saving to: {dirpath}"):
        ...
        with open(label_target_path, "w") as fp:
            ...
            for l, bbox in zip(labels, bboxes):
                ...
                fp.write(to_write)
        _prepare_annotation(annotation, image_target_path, labels, bboxes)
