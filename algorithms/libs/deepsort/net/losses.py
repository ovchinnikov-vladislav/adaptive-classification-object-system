...
def MagnetLoss(margin=1.0, unique_labels=None, only_loss=True):
    def magnet_loss(y_true, y_pred):
        nil = tf.constant(0., tf.float32)
        one = tf.constant(1., tf.float32)
        minus_two = tf.constant(-2., tf.float32)
        eps = tf.constant(1e-4, tf.float32)
        margin_internal = tf.constant(margin, tf.float32)

        num_per_class = None
        unique_labels_internal = unique_labels
        if unique_labels_internal is None:
            unique_labels_internal, sample_to_unique_y, num_per_class = tf.unique_with_counts(tf.reshape(y_true, [-1]))
            num_per_class = tf.cast(num_per_class, tf.float32)

        y_mat = tf.cast(tf.equal(tf.reshape(y_true, (-1, 1)), tf.reshape(unique_labels_internal, (1, -1))), dtype=tf.float32)
        ...
        linear = const * squared_distance - y_mat * margin_internal
