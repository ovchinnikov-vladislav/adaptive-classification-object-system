...
def build_mask(poses, x, classes, dim):
    x = tf.one_hot(x, depth=classes)
    x = tf.tile(tf.reshape(x, (-1, classes, 1)), multiples=[1, 1, dim])
    x = poses * tf.cast(x, dtype=tf.float32)
    x = tf.reshape(x, (-1, classes * dim))
