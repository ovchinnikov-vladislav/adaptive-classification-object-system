...
import colorsys
from libs.yolo.v3.layers import yolo_v3, yolo_v3_tiny
...
class YoloModel:
    def __init__(self, model='yolo3', use_tracking=True, classes=config.coco_classes_ru, size=416):
        ...
        if model == 'yolo3':
            ...
            self.yolo = yolo_v4(anchors, size=size, channels=3, classes=num_classes)
        elif model == 'yolo3_tiny':
            weights = config.yolo_v3_tiny_weights
            anchors = get_anchors(config.yolo_v3_tiny_anchors)
            self.yolo = yolo_v3_tiny(anchors, size=size, channels=3, classes=num_classes)
        elif model == 'yolo2':
