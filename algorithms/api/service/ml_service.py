...
detection_model = ObjectDetectionModel(classes=['person'], use_tracking=False)
video_model = VideoClassCapsNetModel()


def get_video_frame_with_detection(cam, user_id, detection_process_id):
    i = 1
    objects_frame = dict()
    while True:
...
def get_video_with_detection_objects(video_id, user_id, detection_process_id, type='youtube'):
    # detection_model.clear_tracker()

    if type == 'youtube':
        camera = YoutubeCamera(detection_model, video_id)
    elif type == 'video':
        camera = VideoCamera(detection_model, video_id)
    else:
        raise Exception('error detection objects')
    return Response(get_video_frame_with_detection(camera, user_id, detection_process_id),
