...
class GammaCapsule(tf.keras.Model, ABC):
    def __init__(self, capsules, dim_capsules, routings=3, name=''):
    ...
    def call(self, u, training=None, mask=None):
        """
        ...
        u = tf.tile(u, [1, self.capsules, 1, 1, 1])
        u = tf.tile(u, [1, 1, 1, self.dim_capsules, 1])
        ...
        b_ij = tf.zeros(shape=[batch_size, self.capsules, u.shape[1], 1])
        ...
        for r in range(self.routings):
            ...
            if r < self.routings - 1:
                ...
                v_j = tf.tile(v_j, [1, 1, u.shape[1], 1])  # (batch_size, out_caps, in_caps, out_dim)
